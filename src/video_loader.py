...
class MultipleVideoLoader:
    ...
    def fps(self):
        return {src: vc.get(cv2.CAP_PROP_FPS) for src, vc in self._video_captures.items()}
    ...
    def load_next(self):
        next_frame_ids = [
            int(vc.get(cv2.CAP_PROP_POS_FRAMES)) for vc in self._video_captures.values()
        ]
    ...
    def __getitem__(self, index):
        if not all([index < len(video_frames) for video_frames in self._frames.values()]):
            return None
